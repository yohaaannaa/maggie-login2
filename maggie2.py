    ...
    if email and password:
        try:
            ...
            if user_doc.exists:
                ...
                print(f"✅ Sessão salva no Firestore para {email}")  # <-- Testando se está gravando
            ...
            st.error(f"❌ Erro: {e}")
            print(f"❌ Erro ao salvar sessão: {e}")  # <-- Testando se há erro
...
if st.button("Sair"):
    if email:
        try:
            ...
            print(f"❌ Erro ao remover sessão: {e}")  # <-- Testando se há erro
